...
1. **`AudioCue` (ScriptableObject)**: Berfungsi sebagai file "Kaset Data" murni. Menyimpan *array* `AudioClip`, rentang *Pitch* acak, volume, dan pengaturan *Audio Mixer Group*. 
2. **`AudioEventChannel` (ScriptableObject)**: Berfungsi sebagai "Frekuensi Radio" / *Event Bus*. Skrip lain cukup mengirim sinyal ke saluran ini.
3. **`AudioEmitter` (MonoBehaviour)**: Prefab pengeras suara kosong yang ditarik dari *Object Pool*. Ia memutar `AudioCue` dan otomatis mengembalikan dirinya sendiri ke gudang setelah suara selesai berdering.
4. **`AudioSystem` (MonoBehaviour)**: "Otak Sentral" yang duduk di *scene*. Ia mendengarkan sinyal dari `AudioEventChannel` dan memerintahkan `AudioEmitter` untuk memutar suara.
...
1. Klik kanan di *Project Window* -> **Create -> GameSeed -> Audio -> Audio Event Channel**. Beri nama (misal: `SFX_EventChannel`).
2. Klik kanan -> **Create -> GameSeed -> Audio -> Audio Cue**. Beri nama (misal: `SFX_PistolShoot`).
3. Di *Inspector* `SFX_PistolShoot`, masukkan klip audio kamu, lalu centang **Use Random Pitch** agar suaranya bervariasi.

### 2. Setup Prefab & Manajer
1. Buat GameObject kosong bernama `AudioEmitter`, tambahkan komponen `AudioEmitter.cs` (otomatis menambahkan `AudioSource`), lalu jadikan *Prefab* dan hapus dari *scene*.
2. Buat GameObject kosong bernama `AudioSystem` di *scene* dan tambahkan komponen `AudioSystem.cs`.
3. Di *Inspector* `AudioSystem`:
